...
describe 'Commit Message Handler' do
  ...
  it 'should tell me the file name' do
    output = SpecUtils::Capture.stdout { CheckCommit.check commit_file_name }
  ...
  context 'given file content does contain commit pattern' do
    ...
    it 'should succeed' do
      status = CheckCommit.check commit_file_name
  ...
  context 'file content does NOT contain commit pattern' do
    it 'should fail' do
      ...
      expect { CheckCommit.check commit_file_name }.to raise_error(SystemExit, /between 0 and 5/)
    end
  end
