  ...
  describe 'query' do
    context 'missing a commit hash' do
      it 'should raise an error' do
        ...
        expect {@graph_query.query}.to raise_error(/commit hash/)
      end
    end
    ...
    context 'missing a score' do
      it 'should default score to 0' do
        ...
        expect(@graph_query.query).to include "score: 0"
      end
    end

    context 'with a good commit' do
      it 'should have a createCommit query' do
        expect(@graph_query.query).to include "createCommit"
      end

      it 'should have a publishCommit query' do
        expect(@graph_query.query).to include "publishCommit"
      end

      it 'should have an upsertAuthor query' do
        expect(@graph_query.query).to include "upsertAuthor"
      end

      it 'should have a publishAuthor query' do
        expect(@graph_query.query).to include "publishAuthor"
      end
